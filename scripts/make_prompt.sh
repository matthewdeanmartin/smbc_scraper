...
# readonly SCRIPT_NS="paipi_proc"
readonly SRC_DIR="./paipi"
readonly TMP_ROOT="/tmp"
readonly TMP_DIR="${TMP_ROOT}/paipi"
readonly OUTPUT_MD_BASENAME="paipi.flat.md"

# Copy directory safely
paipi_proc::copy_to_tmp() {
  if [[ ! -d "${SRC_DIR}" ]]; then
...
# Run strip-docs for each subdirectory of tmp/paipi
paipi_proc::run_strip_docs() {
  local dir
...
# in the generated Markdown (paipi.flat.md). Other code fences remain untouched.
paipi_proc::clean_flat_md_trailing_double_hash() {
  local output_md
...
main() {
  paipi_proc::copy_to_tmp
  paipi_proc::run_strip_docs
  paipi_proc::confirm "Run coderoller-flatten-repo?"
  paipi_proc::flatten_repo
  # Post-process the flattened Markdown to remove trailing '##' in Python code blocks only
  paipi_proc::clean_flat_md_trailing_double_hash
  paipi_proc::confirm "Delete temporary files in '${TMP_DIR}'?"
  paipi_proc::cleanup_tmp
}
